    ...
    <div class="max-w-2xl mx-auto w-full">
        ...
        <div class="rounded-xl bg-white dark:bg-gray-900 shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10">
                ...
                @if ($record->status === 'completed')
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Extracted Text</label>
                            @if ($record->extracted_text)
                                <button type="button" x-data="{ copied: false }" x-on:click="navigator.clipboard.writeText(document.getElementById('ocr-text').innerText); copied = true; setTimeout(() => copied = false, 1500)" class="inline-flex items-center gap-1.5 text-sm font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 transition">
                                    <template x-if="!copied">
                                        <span class="inline-flex items-center gap-1">
                                            <x-filament::icon icon="heroicon-o-document-duplicate" class="w-3.5 h-3.5" />
                                            Copy
                                        </span>
                                    </template>
                                    <template x-if="copied">
                        ...
                        <div class="block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-white shadow-sm min-h-[6rem]">
                            <pre id="ocr-text" class="font-sans text-sm leading-relaxed text-gray-800 dark:text-gray-200 m-0 text-left whitespace-pre-wrap break-words" style="white-space: pre-wrap; word-wrap: break-word; overflow-wrap: anywhere;">{{ $record->extracted_text ?: 'No text was extracted.' }}</pre>
                        </div>
                    </div>
                @elseif ($record->status === 'queued' || $record->status === 'processing')
            ...
            <div class="px-6 py-4 border-t border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-gray-800/30 rounded-b-xl flex items-center justify-end gap-3">
                <a href="{{ \App\Filament\Resources\OcrTaskResource::getUrl('edit', ['record' => $record]) }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-white dark:bg-gray-800 px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-200 shadow-sm ring-1 ring-gray-300 dark:ring-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <x-filament::icon icon="heroicon-o-pencil-square" class="w-4 h-4" />
                    Edit
                </a>
                <a href="{{ \App\Filament\Resources\OcrTaskResource::getUrl('edit', ['record' => $record]) }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 transition">
